...
database.ref("/home1/Nhiet Do").on("value",function(snapshot){
    var temp = snapshot.val();
    document.getElementById("temp").innerHTML = temp;
});
database.ref("/home1/Do Am").on("value",function(snapshot){
    var temp = snapshot.val();
...
for( var i = 0;i<tb2.length;i++){
    var c = tb2[1].querySelector('input')
        firebase.database().ref('home1/open/').once('value', function(snap){
    ...
    c.onchange = function(e){
        if(e.target.checked){
            if(e.target.checked){
                ...
                var tb7 = document.querySelectorAll('.devcie');
                var sen_ven = tb7[6];
                if(sen_ven.classList.contains('change')){
                    sen_ven.classList.remove('change');
                }
                else{
                    sen_ven.classList.add("change");
                }
                var dis3 = sen_ven.querySelector('.dis-not')
                dis3.style.display = "none";
                var load3 = sen_ven.querySelector('.load')
                load3.style.display = "block";
                var icon3 = six.querySelector('.icon');
                icon3.classList.add('back');
            }
                ...
                var dis2 = sen_ven.querySelector('.dis-not')
                dis2.style.display = "block";
                var load2 = sen_ven.querySelector('.load')
                load2.style.display = "none";
                var icon3 = sen_ven.querySelector('.icon');
                icon3.classList.remove('back')  
            }
